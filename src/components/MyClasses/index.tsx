...
import { MyClassesMock } from "@/mocks";
...
const MyClasses: React.FC = () => {
  ...
  return (
    <div className="bg-white border-stroke rounded-3xl">
      ...
      {isMobile ? (
        <div className="pb-10 px-8 items-center">
          <div className="flex items-center pl-2">
            <span className="text-xs font-normal text-typographyBody">
              {format(new Date(MyClassesMock[0].date), "dd 'de' MMMM',' yyyy", {
                locale: pt,
              })}
            </span>
          ...
          <div className="items-center pl-2 pt-5">
            <span className="text-xs font-bold text-typographyBody">
              {MyClassesMock[0].title}
            </span>
            <div className="flex">
              <CheckMark />
              <span className="text-xs font-normal text-typographyBody">
                {MyClassesMock[0].views}
              </span>
            </div>
          </div>
          <div className="flex justify-center items-center pt-2">
            <span className="flex justify-center text-xs font-medium text-white bg-blue px-4 py-2 rounded-3xl h-[36px] w-[95%]">
              {MyClassesMock[0].category}
            </span>
          </div>
        </div>
      ) : (
        MyClassesMock.map((data) => {
          return (
            <div
              key={MyClassesMock.indexOf(data)}
